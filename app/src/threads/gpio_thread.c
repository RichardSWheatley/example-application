...
void gpio_thread(void)
{
    ...
    if (status != 0) {
        LOG_ERR("Error %d: failed to configure %s pin %d", status, button.port->name, button.pin);
    }

    status = gpio_pin_interrupt_configure_dt(&button,
                          GPIO_INT_EDGE_TO_ACTIVE);
    if (status != 0) {
        LOG_ERR("Error %d: failed to configure interrupt on %s pin %d", status, button.port->name, button.pin);
    }
    ...
    LOG_ERR("Error %d: LED device %s is not ready; ignoring it", status, led.port->name);
        led.port = NULL;
    }
    if (led.port) {
        status = gpio_pin_configure_dt(&led, GPIO_OUTPUT);
        if (status != 0) {
            LOG_ERR("Error %d: failed to configure LED device %s pin %d", status, led.port->name, led.pin);
            led.port = NULL;
        } else {
